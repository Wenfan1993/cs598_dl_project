...
#%%
#-------- deconv module --------#
def deconv(in_channels, out_channels, output_padding=1):
    return nn.Sequential(
        nn.ConvTranspose2d(in_channels, out_channels, (3,3), (2,2), padding=(1,1), output_padding=(output_padding,output_padding)),        
    )   
...
def conv_out(in_channels, out_channels):
    return nn.Sequential(
        nn.Conv2d(in_channels, out_channels, (1,1), padding=(0,0)),
        nn.LeakyReLU(0.2,inplace=True)
    )   
...
class Unet(nn.Module):
    def __init__(self, n_input = 1, n_out=2, n_h=260, n_w=311):
        super().__init__()
        
        
        # calculate output_padding for decon
        self.nx_l1 = n_h
        self.nx_l2 = int(np.ceil(self.nx_l1/2))
        self.nx_l3 = int(np.ceil(self.nx_l2/2))
        
        self.nx_l1_w = n_w
        self.nx_l2_w = int(np.ceil(self.nx_l1_w/2))
        self.nx_l3_w = int(np.ceil(self.nx_l2_w/2))
        
        # initialize layers
        self.conv_down1 = quad_conv(n_input, 16)
        self.conv_down2 = quad_conv(16, 32)
        self.conv_down3 = quad_conv(32, 64)
        self.conv_down4 = quad_conv(64, 128)
        ...
        self.conv_up1   = quad_conv(128, 64)
        self.conv_up2   = quad_conv(64, 32)
        self.conv_up3   = quad_conv(32, 16)
        
        self.conv_out   = conv_out(16, n_out)
    ...
    def forward(self,x):
        ...
        ## first up-block
        x     = self.deconv1(x)
        x     = x[:,:,0:int(self.nx_l3),0:int(self.nx_l3_w)]
        x     = torch.cat((x,conv3),dim=1)
        x     = self.conv_up1(x)
            
        ## second up-block
        x     = self.deconv2(x)
        x     = x[:,:,0:int(self.nx_l2),0:int(self.nx_l2_w)]
        x     = torch.cat((x,conv2),dim=1)
        x     = self.conv_up2(x)        
        
        ## third up-block
        x     = self.deconv3(x)
        x     = x[:,:,0:int(self.nx_l1),0:int(self.nx_l1_w)]
