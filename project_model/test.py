...
from util.util import tensor2mat
...
if __name__ == '__main__':
    ...
    real_A = np.zeros((opt.num_test, 260, 311))
    real_B = np.zeros((opt.num_test, 260, 311))
    fake_B = np.zeros((opt.num_test, 260, 311))
    ...
    for i, data in enumerate(dataset):
        ...
        if opt.dataset_mode == 'templatewholehcp':
            for label, im_data in items_save:
                mat = tensor2mat(im_data[0, :, :, :].reshape((1, -1, 260, 311)))
    ...
    #     index[i, 0] = start_idx[i][0]
    #     index[i, 1] = start_idx[i][1]
    #     index[i, 2] = start_idx[i][2]
